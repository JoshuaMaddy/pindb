...
- A pending **edit chain** gets its own bar on the same five pages (`edit_review_actions_bar`, same module), posting to the `/admin/pending/{approve,reject,delete}-edits/…` routes with **`?after=reload`**. It is separate from `review_actions_bar` because the two states are independent — an approved pin can carry a pending edit, so both bars can legitimately show at once — and because Delete here discards the submission, not the entry. `after=reload` rather than `after=back`: ruling on an edit leaves the page standing (approving writes the new values into the entity the admin is reading), so the right ending is htmx's own `HX-Refresh: true` re-render in place, with no shell listener involved. Needs-changes chains get Approve + Discard only.
...
- **"Pins in this display" is one strip per photo, not one deduped strip for the whole display** (`_tagged_pins_section`/`_image_pin_strip` in `display_page.py`). A pin tagged on two photos legitimately appears twice — the photo is the context for "why is this pin here," so collapsing repeats across photos would throw that away. The strip's own heading count is a sum across photos, not a distinct-pin count, for the same reason.
...
- **Layouts** (`display_layouts.py`): `grid` / `vertical` / `carousel`, plus a per-image `size_hint` (`normal`/`wide`/`tall`/`large` = 1x1/2x1/1x2/2x2 tiles), read only by `grid` — the editor island only shows the size picker while `grid` is the selected layout. `grid` is a CSS **grid** with `grid-flow-dense`, not columns-masonry — multi-column CSS cannot span an item across columns. There's no 3-tile option: every shape is a rectangle a single grid item can cover, and no rectangle spans exactly 3 cells. Row height is pinned via a container-query `calc()` on `grid-auto-rows` (`[container-type:inline-size]` + `calc((100cqw-gap)/cols)`) rather than left to content/`auto` — that's what makes a `row-span-2` tile exactly two column-widths tall even when nothing else shares its row; `auto` only gets that right by coincidence, when a neighboring 1x1 tile happens to size the row for it. In the fixed-height `grid` layout the caption is **overlaid on the photo** (a below-image caption spills out of its cell); `vertical` keeps it below. `carousel` needs both `swiper.min.js` **and** `swiper.min.css` (`display_page.py` head_content) — without the stylesheet `.swiper-wrapper`/`.swiper-slide` have no flex layout or overflow clipping and slides stack instead of sliding. Same contract on the pin detail page's carousel (`templates/get/pin.py`). Carousel main-slide height fills the remaining viewport (`display_swiper.js` sets it via `getBoundingClientRect`, recomputed on resize) — the Tailwind height classes on `.display-swiper-main` are only the pre-JS/no-JS fallback. **`object_fit`** (cover/contain/fill) is a separate, always-visible-in-the-editor per-image choice, orthogonal to `size_hint` and applied in every layout (not grid-only). It's set once at upload time from the display's *current* layout (`cover` for `grid`, `contain` otherwise, matching what those layouts always rendered before this was overridable) and is freely editable afterwards; changing the display's layout later does not retroactively touch already-uploaded photos.
