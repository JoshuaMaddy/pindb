...
async def approve_entity(
    ...
    async with async_session_maker.begin() as session:
        ...
        if entity_type == EntityType.pin:
            pin_with_rels = await session.scalar(
                select(Pin)
...
async def approve_pending_edits(
    ...
    async with async_session_maker.begin() as session:
        entity = await _load_entity_for_edit(session, entity_type, entity_id)
        if entity is None:
            raise HTTPException(status_code=404, detail="Entity not found")
...
async def _collect_bulk_entities(
    session: AsyncSession, bulk_id: UUID
...
async def approve_bulk(
    ...
    async with async_session_maker.begin() as session:
        ...
        for (table_name, entity_id), chain in edits_by_entity.items():
            ...
            if entity_type is None:
                continue
            canonical = await _load_entity_for_edit(session, entity_type, entity_id)
            ...
            full_chain = await get_edit_chain(session, table_name, entity_id)
            effective = get_effective_snapshot(canonical, full_chain)
            await apply_snapshot_to_entity(canonical, effective, session)
