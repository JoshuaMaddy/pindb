...
def bulk_tags_page(
    ...
    *,
    ...
    bread: list[BreadCrumbLink | str] = [
        (request.url_for("get_admin_panel"), "Admin"),
        "Bulk tags",
    ]
    body_blocks: list[object] = [
    ...
    if error_message:
        body_blocks.append(
            div(class_="alert alert-error max-w-2xl text-sm whitespace-pre-wrap")[
                error_message
            ]
        )
    if result is not None:
        body_blocks.append(
            div(class_="alert alert-success max-w-2xl text-sm")[
    ...
    return html_base(
        title="Bulk tags",
        request=request,
        bread_crumb_links=bread,
        body_content=centered_div(
            content=cast(Content, body_blocks),
            flex=True,
            col=True,
        ),
    )
