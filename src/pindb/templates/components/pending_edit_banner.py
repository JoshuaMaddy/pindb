...
def pending_edit_banner(
    *,
    ...
    if viewing_pending:
        return div(
            class_="rounded bg-error-dark border border-error-dark text-error-main px-4 py-2 text-sm my-2 flex items-center gap-2"
        )[
            i(data_lucide="clock", class_="inline-block w-4 h-4", aria_hidden="true"),
            "Viewing pending edit.",
            a(
                href=canonical_url,
                class_="underline text-error-main-hover hover:text-error-main ml-auto",
            )["View canonical →"],
        ]
    return div(
        class_="rounded bg-error-dark border border-error-dark text-error-main px-4 py-2 text-sm my-2 flex items-center gap-2"
