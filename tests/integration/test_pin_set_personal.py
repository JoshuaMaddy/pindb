...
class TestPersonalSetCrud:
    def test_authenticated_user_can_create_personal_set(
        ...
        pin_set = db_session.scalar(
            select(PinSet)
            .where(PinSet.name == "My Fav Pins")
            .execution_options(include_pending=True)
        )
        assert pin_set is not None
        assert pin_set.owner_id == test_user.id
    ...
    def test_owner_can_delete_personal_set(self, auth_client, db_session, test_user):
        pin_set = PersonalPinSetFactory(owner_id=test_user.id)
    ...
    def test_other_user_cannot_delete_someone_elses_set(
        self, auth_client, db_session, other_editor_user
    ):
...
class TestPersonalSetMembership:
    def test_add_pin_then_reorder(self, auth_client, db_session, test_user, admin_user):
        pin_set = PersonalPinSetFactory(owner_id=test_user.id)
...
class TestPromoteToGlobal:
    def test_admin_can_promote_personal_set(self, admin_client, db_session, test_user):
        pin_set = PersonalPinSetFactory(owner_id=test_user.id)
        set_id = pin_set.id  # ty:ignore[unresolved-attribute]
    ...
    def test_regular_user_cannot_promote(self, auth_client, db_session, test_user):
        pin_set = PersonalPinSetFactory(owner_id=test_user.id)
        response = auth_client.post(
