...
class TestLegalPages:
    ...
    def test_legal_pages_return_200(self, auth_client, path: str):
...
class TestTagHelperRoutes:
    ...
    def test_tag_implication_preview_empty_when_no_tags(self, auth_client):
        response = auth_client.get("/get/tag-implication-preview")
        assert response.status_code == 200
...
class TestDocsRoutes:
    def test_docs_index_returns_200(self, auth_client):
        response = auth_client.get("/docs")
        assert response.status_code == 200
        assert "<html" in response.text.lower()

    def test_docs_section_redirects_to_first_page(self, auth_client):
        ...
        response = auth_client.get(f"/docs/{section_key}", follow_redirects=False)
    ...
    def test_unknown_section_returns_404(self, auth_client):
        response = auth_client.get("/docs/not-a-section", follow_redirects=False)
        assert response.status_code == 404

    def test_unknown_page_returns_404(self, auth_client):
        docs_module = importlib.import_module("pindb.routes.docs")
        section_key = next(iter(docs_module._SECTIONS))
        response = auth_client.get(f"/docs/{section_key}/not-a-real-page")
        assert response.status_code == 404


@pytest.mark.integration
class TestTagRelationsRoute:
    def test_relations_render_implications(self, auth_client, db_session, admin_user):
        implied = TagFactory(
    ...
    def test_invalid_direction_returns_empty(self, auth_client, db_session, admin_user):
    ...
    def test_unknown_tag_returns_empty(self, auth_client):
        response = auth_client.get("/get/tag/9999999/relations/implications")
        assert response.status_code == 200
        assert response.text == ""
